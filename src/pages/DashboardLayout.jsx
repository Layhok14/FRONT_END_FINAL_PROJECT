                  ...
                  <div key={phase.id} className="rounded-[18px] border border-line bg-white p-4">
                    ...
                    <div className="mt-3 space-y-3">
                      {phase.items.length ? phase.items.map((item) => {
                        ...
                        return (
                          <div key={item.medicationId} className="flex items-center justify-between gap-3 rounded-2xl border border-line bg-surface-soft px-4 py-3 text-sm">
                            <div>
      ...
      <Modal open={addCaregiverOpen} onClose={() => setAddCaregiverOpen(false)} title="Add caregiver request">
        <form className="space-y-4" onSubmit={addCaregiverRequest}>
          ...
          <label className="block">
            ...
            <textarea className="min-h-25 w-full rounded-2xl border border-line px-4 py-3 outline-none focus:border-primary" value={caregiverForm.note} onChange={(event) => setCaregiverForm((current) => ({ ...current, note: event.target.value }))} />
          </label>
