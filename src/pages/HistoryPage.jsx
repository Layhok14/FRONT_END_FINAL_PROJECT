...
import { getMealSectionStatus } from '../utils/dashboard'
        ...
        <div className="flex flex-wrap items-center gap-3 rounded-full bg-surface-soft p-1">
          {['records', 'past-medications'].map((mode) => (
            <button
              ...
              className={`rounded-full px-4 py-2 text-sm font-semibold ${historyMode === mode ? 'bg-primary text-white' : 'text-text-soft'}`}
            >
          ...
          <section className="rounded-[28px] border border-line bg-white p-5 shadow-[var(--shadow)]">
            <div className="grid gap-4 md:grid-cols-[1fr_220px]">
              <input
                ...
                placeholder="Search past medicine name"
              />
              <select
              ...
              >
                ...
                {[...new Set(pastMedications.map((medication) => medication.category))].map((category) => (
